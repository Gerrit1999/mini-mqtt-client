...
2. **接收后处理（after_receive）**
   ...
   - 可访问全局变量：`payloadBytes`（Uint8Array，原始 MQTT payload 字节）
   - 应用场景：数据解析、解密、格式化展示等
...
| 函数 | 说明 |
|------|------|
| `pako.gzip(data, options?)` | 原生 gzip 压缩，返回 Uint8Array |
| `pako.ungzip(data, options?)` | 原生 gzip 解压 |
| `pako.deflate(data, options?)` | 原生 zlib(deflate) 压缩 |
| `pako.inflate(data, options?)` | 原生 zlib(inflate) 解压 |
...
```javascript
async function process(payload, topic) {
  // payloadBytes 是原始 MQTT 字节，适合处理 gzip/zlib 二进制
  const text = crypto.bytesToString(pako.ungzip(payloadBytes));
  return text;
}
```